...
export type UiCopy = {
  ...
  services: { title: string; badge: string; hero1: string; hero2: string; heroSub: string; view: string; more: string; areaBadge: string; choose: string; yourService: string; learnMore: string; ctaBadge: string; ctaTitle: string };
  ...
  auth: {
    ...
    oauthCompleting: string; oauthFailed: string; oauthNotConfigured: string; clientAccountRequired: string; backHome: string;
  ...
  request: {
    ...
    // Recruitment
    recruitmentTypeLabel: string; namedCandidates: string; namedCandidatesDesc: string;
    recruitmentKind: string; recruitmentKindDesc: string; generalKind: string; generalKindDesc: string;
    vacancyTitle: string; vacancyCount: string; targetCountry: string; additionalRequirements: string;
    requiredNationality: string; requiredProfession: string;
    candidatesSection: string; addCandidate: string; candidateName: string; candidatePhone: string; candidateEmail: string;
    candidateNationality: string; candidateCurrentProfession: string; candidateDesiredProfession: string;
    candidateWorkStatus: string; candidateCountry: string; candidateNotes: string;
    workStatusEmployed: string; workStatusUnemployed: string; workStatusFreelance: string;
    workStatusStudent: string; workStatusOther: string;
    candidateIsGulfResident: string; candidateEducationLevel: string;
    eduBelow: string; eduSecondary: string; eduDiploma: string; eduBachelor: string;
    eduMaster: string; eduDoctorate: string; eduOther: string;
    docsSection: string; docsComplete: string; docsMissing: string;
    docPassport: string; docEducation: string; docResidence: string;
    docResignation: string; docExperience: string;
    docUpload: string; docUploading: string; docUploaded: string;
    docUploadError: string; docSizeError: string; docTypeError: string;
    docRequired: string; docOptional: string;
    candidateDeleted: string; candidateDeleteError: string;
    finishNoCandidates: string; finishDocsIncomplete: string;
    savingCandidate: string; candidateAdded: string; candidateAddError: string; candidateCount: string;
    candidateStatus: string;
    candidateStatusSubmitted: string; candidateStatusUnderReview: string; candidateStatusShortlisted: string;
    candidateStatusInterviewScheduled: string; candidateStatusOfferExtended: string;
    candidateStatusHired: string; candidateStatusRejected: string; candidateStatusWithdrawn: string;
    adminAddCandidateTitle: string; adminCandidatePublicStatus: string; adminCandidateInternalNotes: string;
    candidateClientVisible: string; candidateClientVisibleHint: string;
    updateCandidate: string; candidateUpdated: string; candidateUpdateError: string;
    recruitmentInfo: string; recruitmentType: string;
  };
  packages: {
...
const en: UiCopy = {
  ...
  about: {
    ...
    ctaBadge: "Let's create impact together", requestService: "Request a service",
  },
  contact: {
  ...
  request: { title: "New service request", subtitle: "Complete the form and our team will contact you within 24–48 hours", steps: ["Company information", "Contact details", "Service details", "Review"], company: "Company name", commercialReg: "Commercial registration (optional)", activity: "Business activity", contactEmail: "Contact email", contactPhone: "Phone number", service: "Requested service", package: "Package (optional)", country: "Recruitment country (if applicable)", notes: "Additional notes (optional)", chooseService: "Choose a service...", chooseCountry: "Choose a country...", next: "Next", previous: "Previous", review: "Review request", submit: "Submit request", submitted: "Submitted", success: "Your request was submitted successfully.", error: "Could not submit the request.", required: "Required", recruitmentTypeLabel: "Recruitment request type", namedCandidates: "Named candidates", namedCandidatesDesc: "I have specific candidates to submit", recruitmentKind: "Vacancy / sourcing", recruitmentKindDesc: "I need OFOQ to source candidates for a vacancy", generalKind: "General HR management", generalKindDesc: "Standard HR service request without recruitment", vacancyTitle: "Job title", vacancyCount: "Number of positions", targetCountry: "Recruitment country", additionalRequirements: "Requirements and notes", requiredNationality: "Required nationality", requiredProfession: "Required profession", candidatesSection: "Candidates", addCandidate: "Add candidate", candidateName: "Full name", candidatePhone: "Phone number", candidateEmail: "Email (optional)", candidateNationality: "Nationality", candidateCurrentProfession: "Current profession", candidateDesiredProfession: "Desired profession", candidateWorkStatus: "Work status", candidateCountry: "Country of residence", candidateNotes: "Notes (optional)", workStatusEmployed: "Employed", workStatusUnemployed: "Seeking work", workStatusFreelance: "Freelance", workStatusStudent: "Student", workStatusOther: "Other", candidateIsGulfResident: "Currently residing in a Gulf country", candidateEducationLevel: "Education level", eduBelow: "Below secondary", eduSecondary: "Secondary", eduDiploma: "Diploma", eduBachelor: "Bachelor", eduMaster: "Master", eduDoctorate: "Doctorate", eduOther: "Other", docsSection: "Required documents", docsComplete: "All documents uploaded", docsMissing: "Documents pending upload", docPassport: "Passport", docEducation: "Education certificate", docResidence: "Residence permit", docResignation: "Resignation acknowledgement", docExperience: "Experience certificate", docUpload: "Upload", docUploading: "Uploading...", docUploaded: "Uploaded", docUploadError: "Upload failed", docSizeError: "File must be under 5 MB", docTypeError: "Only PDF, JPEG, or PNG files are accepted", docRequired: "Required", docOptional: "Optional", candidateDeleted: "Candidate removed", candidateDeleteError: "Could not remove candidate", finishNoCandidates: "Add at least one candidate before continuing.", finishDocsIncomplete: "Upload all required documents before continuing. Missing:", savingCandidate: "Saving...", candidateAdded: "Candidate added", candidateAddError: "Could not add candidate", candidateCount: "candidates", candidateStatus: "Status", candidateStatusSubmitted: "Submitted", candidateStatusUnderReview: "Under review", candidateStatusShortlisted: "Shortlisted", candidateStatusInterviewScheduled: "Interview scheduled", candidateStatusOfferExtended: "Offer extended", candidateStatusHired: "Hired", candidateStatusRejected: "Rejected", candidateStatusWithdrawn: "Withdrawn", adminAddCandidateTitle: "Add candidate (sourcing)", adminCandidatePublicStatus: "Client-visible status", adminCandidateInternalNotes: "Internal notes (admin only)", candidateClientVisible: "Visible to client", candidateClientVisibleHint: "When enabled, the client can see this candidate's safe progress (name, profession, status). Contact details and internal notes are never shared.", updateCandidate: "Update", candidateUpdated: "Candidate updated", candidateUpdateError: "Could not update candidate", recruitmentInfo: "Recruitment details", recruitmentType: "Request type" },
...
const ar: UiCopy = {
  ...
  about: {
    ...
    ctaBadge: "معاً نصنع الأثر", requestService: "اطلب خدمة",
  },
  contact: {
  ...
  request: { title: "طلب خدمة جديد", subtitle: "أكمل النموذج وسيتواصل فريقنا معك خلال 24–48 ساعة", steps: ["معلومات الشركة", "بيانات التواصل", "تفاصيل الخدمة", "المراجعة"], company: "اسم الشركة", commercialReg: "رقم السجل التجاري (اختياري)", activity: "النشاط التجاري", contactEmail: "البريد الإلكتروني للتواصل", contactPhone: "رقم الهاتف", service: "نوع الخدمة المطلوبة", package: "الباقة (اختياري)", country: "دولة الاستقدام (إن انطبق)", notes: "ملاحظات إضافية (اختياري)", chooseService: "اختر الخدمة...", chooseCountry: "اختر الدولة...", next: "التالي", previous: "السابق", review: "مراجعة الطلب", submit: "إرسال الطلب", success: "تم تقديم طلبك بنجاح!", error: "خطأ في إرسال الطلب", required: "مطلوب", recruitmentTypeLabel: "نوع طلب الموارد البشرية", namedCandidates: "مرشحون محددون", namedCandidatesDesc: "لدي مرشحون محددون لتقديمهم", recruitmentKind: "توظيف واستقطاب", recruitmentKindDesc: "أحتاج أفق لاستقطاب مرشحين لشغل وظيفة", generalKind: "إدارة موارد بشرية عامة", generalKindDesc: "طلب خدمة موارد بشرية عادي دون استقطاب", vacancyTitle: "المسمى الوظيفي", vacancyCount: "عدد الوظائف المطلوبة", targetCountry: "دولة الاستقطاب", additionalRequirements: "المتطلبات والملاحظات", requiredNationality: "الجنسية المطلوبة", requiredProfession: "التخصص المطلوب", candidatesSection: "المرشحون", addCandidate: "إضافة مرشح", candidateName: "الاسم الكامل", candidatePhone: "رقم الهاتف", candidateEmail: "البريد الإلكتروني (اختياري)", candidateNationality: "الجنسية", candidateCurrentProfession: "المسمى الوظيفي الحالي", candidateDesiredProfession: "المسمى الوظيفي المطلوب", candidateWorkStatus: "حالة العمل", candidateCountry: "دولة الإقامة", candidateNotes: "ملاحظات (اختياري)", workStatusEmployed: "موظف", workStatusUnemployed: "يبحث عن عمل", workStatusFreelance: "حر", workStatusStudent: "طالب", workStatusOther: "أخرى", candidateIsGulfResident: "مقيم حالياً في دولة خليجية", candidateEducationLevel: "المستوى التعليمي", eduBelow: "دون الثانوية", eduSecondary: "ثانوية عامة", eduDiploma: "دبلوم", eduBachelor: "بكالوريوس", eduMaster: "ماجستير", eduDoctorate: "دكتوراه", eduOther: "أخرى", docsSection: "الوثائق المطلوبة", docsComplete: "تم رفع جميع الوثائق", docsMissing: "وثائق معلقة", docPassport: "جواز السفر", docEducation: "الشهادة التعليمية", docResidence: "تصريح الإقامة", docResignation: "إقرار الاستقالة", docExperience: "شهادة الخبرة", docUpload: "رفع", docUploading: "جاري الرفع...", docUploaded: "تم الرفع", docUploadError: "فشل الرفع", docSizeError: "يجب أن لا يتجاوز حجم الملف 5 ميغابايت", docTypeError: "يُقبل فقط ملفات PDF أو JPEG أو PNG", docRequired: "مطلوب", docOptional: "اختياري", candidateDeleted: "تم حذف المرشح", candidateDeleteError: "تعذر حذف المرشح", finishNoCandidates: "أضف مرشحاً واحداً على الأقل قبل المتابعة.", finishDocsIncomplete: "ارفع جميع الوثائق المطلوبة قبل المتابعة. المفقود:", savingCandidate: "جاري الحفظ...", candidateAdded: "تمت إضافة المرشح", candidateAddError: "تعذر إضافة المرشح", candidateCount: "مرشح", candidateStatus: "الحالة", candidateStatusSubmitted: "مُقدَّم", candidateStatusUnderReview: "قيد المراجعة", candidateStatusShortlisted: "في قائمة المختارين", candidateStatusInterviewScheduled: "مقابلة مجدولة", candidateStatusOfferExtended: "تم تقديم عرض", candidateStatusHired: "تم التعيين", candidateStatusRejected: "مرفوض", candidateStatusWithdrawn: "انسحب", adminAddCandidateTitle: "إضافة مرشح (للاستقطاب)", adminCandidatePublicStatus: "الحالة المرئية للعميل", adminCandidateInternalNotes: "ملاحظات داخلية (للإدارة فقط)", candidateClientVisible: "مرئي للعميل", candidateClientVisibleHint: "عند التفعيل يتمكن العميل من رؤية التقدم الآمن لهذا المرشح (الاسم والمهنة والحالة). لا تُشارَك بيانات التواصل أو الملاحظات الداخلية مطلقاً.", updateCandidate: "تحديث", candidateUpdated: "تم تحديث المرشح", candidateUpdateError: "تعذر تحديث المرشح", recruitmentInfo: "تفاصيل التوظيف", recruitmentType: "نوع الطلب" },
...
const overrides: Record<Exclude<Lang, "ar" | "en">, DeepPartial<UiCopy>> = {
  ur: {
    header: { clientLogin: "کلائنٹ لاگ اِن", menu: "مینو", language: "زبان", nav: ["ہوم", "ہمارے بارے میں", "خدمات", "پیکیجز", "ممالک", "بلاگ", "رابطہ"] },
    home: {
